...
public class LoginPasswordField extends BorderPane {
    ...
    public LoginPasswordField(){
        ...
        textField.addEventHandler(MouseEvent.MOUSE_EXITED, event -> {
            if (!textField.isFocused()) {
                textField.setStyle("-fx-prompt-text-fill: #161E33");
            }
        });
        ...
        passwordMainField.addEventHandler(MouseEvent.MOUSE_ENTERED, event -> {
            if(!passwordField.isFocused() & !textField.isFocused()){
                ...
                passwordMainField.setStyle("-fx-border-color: #BDD3FF");
                lock.setStyle("-fx-text-fill: #BDD3FF");
                passwordField.setStyle("-fx-prompt-text-fill: #BDD3FF");
            }
        });
        // 监听器，鼠标离开时恢复原样式
        passwordMainField.addEventHandler(MouseEvent.MOUSE_EXITED, event -> {
            if(!passwordField.isFocused() & !textField.isFocused()){
                passwordMainField.setStyle("-fx-border-color: #161E33");
                lock.setStyle("-fx-text-fill: #161E33");
                passwordField.setStyle("-fx-prompt-text-fill: #161E33");
            }
        });
        // 加载字体图标

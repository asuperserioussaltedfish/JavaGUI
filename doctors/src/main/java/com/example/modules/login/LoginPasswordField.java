    ...
    public LoginPasswordField(){
        ...
        passwordMainField.addEventHandler(MouseEvent.MOUSE_ENTERED, event -> {
            if(!passwordField.isFocused() & !textField.isFocused()){
                ...
                passwordMainField.setStyle("-fx-border-color: #E0CEFF");
                lock.setStyle("-fx-text-fill: #E0CEFF");
                passwordField.setStyle("-fx-prompt-text-fill: #E0CEFF");
            }
        });
        // 监听器，鼠标离开时恢复原样式
        passwordMainField.addEventHandler(MouseEvent.MOUSE_EXITED, event -> {
            if(!passwordField.isFocused() & !textField.isFocused()){
                passwordMainField.setStyle("-fx-border-color: #4A004A");
                lock.setStyle("-fx-text-fill: #4A004A");
                passwordField.setStyle("-fx-prompt-text-fill: #4A004A");
            }
        });
        // 加载字体图标
